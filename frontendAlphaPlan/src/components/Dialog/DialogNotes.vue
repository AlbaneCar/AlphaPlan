...
import { fetchNoteByTypeAndNames } from '../../services/NotesService.js';
...
const loadData = async () => {
    const data = await fetchNoteByTypeAndNames(props.nom, props.prenom, props.sprintId, props.type);
    notes.value = data;
};
...
<template>
    <div>
        <Dialog v-model:visible="visible" modal class="w-2/5 h-fit" :closable="false"
        :header="'Détails de la note \'' + NAMES[type.toUpperCase()] + '\' pour '+ prenom + ' ' + nom">
            ...
            <div class="flex flex-col justify-center items-center">
                ...
                <Carousel v-else :value="notes" :numVisible="2" :numScroll="1" :showIndicators="false" class="w-full">
                    <template #item="note">
                        <div class="flex flex-col border rounded-md shadow-md p-5 m-5 h-3/4">
                            ...
                            <div class="flex flex-row w-full h-full justify-center items-center">
                                ...
                                <div class="italic p-2 w-full max-h-[30vh] overflow-y-auto break-all">
                                    <p>{{ (note.data.commentaire) ? note.data.commentaire : 'Pas de commentaire.' }}</p>
                                </div>
                            </div>
                        </div>
                    </template>
